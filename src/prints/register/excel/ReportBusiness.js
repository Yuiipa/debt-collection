...
export async function generateExcel(item) {
  ...
  worksheet.getCell('A1').value = 'รายงานข้อมูลสถานประกอบธุรกิจทวงถามหนี้'
  ...
  worksheet.getCell('A2').value = `จังหวัด${
    item[0].location.split('จังหวัด')[1]
  }`
  ...
  worksheet.getCell('A3').value = 'กรมการปกครอง กระทรวงมหาดไทย'
  ...
  item.forEach((data, index) => {
    const row = worksheet.addRow([
      ...
      ThaiNumbers(data.registrationNumber), // เลขทะเบียนที่
  ...
  const totalRow3 = worksheet.addRow([
    ...
    '',
    ThaiNumbers(BOnaturalCount || 0),
    'ผู้ประกอบธุรกิจที่เป็นนิติบุคคล',
    '',
    ThaiNumbers(BOjuristicCount || 0),
  ])
  worksheet.mergeCells(
    `B${worksheet.lastRow.number}:C${worksheet.lastRow.number}`
  )
